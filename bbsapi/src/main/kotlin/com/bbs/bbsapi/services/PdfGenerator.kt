...
class PdfGenerator {
    ...
    fun generateInvoicePdf(invoice: PdfInvoiceDTO): ByteArray {
        ...
        val totalTable = PdfPTable(2).apply {
            ...
            addCell(createCell("kshs ${invoice.total}", normalFont))
            addCell(createCell("TOTAL", companyBoldFont, horizontalAlignment = Element.ALIGN_RIGHT))
            addCell(createCell("kshs ${invoice.total}", companyBoldFont))
        }
        document.add(totalTable)

        // Total in Words (Right-aligned)
        val totalWords = Paragraph("${convertToWords(invoice.total.toInt())} Kenya shillings only.", normalFont).apply {
